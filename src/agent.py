...
class AgentState(TypedDict):
    ...
    # Track actions taken with operator.add reducer to accumulate actions
...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    prompt = prompt_template.format(
        ...
        conversation_history="\n".join([str(msg) for msg in history]) if history else "No previous conversation"
    )

    # Invoke the LLM to classify intent
    intent = structured_llm.invoke(prompt)

    # Set next_step based on classified intent
    if intent.intent_type == "qa":
        ...
        next_step = "qa_agent"  # Default to qa_agent
...
def create_workflow(llm, tools):
    ...
    # Compile with InMemorySaver checkpointer to persist state
