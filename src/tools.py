...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A mathematical expression to evaluate (e.g., "2 + 2", "100 * 0.15", "50000 + 30000")
        ...
        try:
            ...
            for pattern in dangerous_patterns:
                if pattern in expression.lower():
                    error_msg = f"Invalid expression: contains forbidden pattern '{pattern}'"
            ...
            if not all(c in allowed_chars for c in expression):
                error_msg = "Invalid expression: contains forbidden characters. Only numbers and basic operators (+, -, *, /, parentheses) are allowed."
                logger.log_tool_use("calculator", {"expression": expression}, {"error": error_msg})
                return error_msg
